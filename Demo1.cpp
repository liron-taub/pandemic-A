...
int main() {
    ...
    OperationsExpert player{board,
                            City::Atlanta};  // initialize an "operations expert" player on the given board, in Atlanta.
    ...
    try {
        player.fly_charter(
                City::Seoul);  // illegal action: you do not have the Sydney card (the card of the city you are in).
    } catch (const exception &ex) {
        cout << "   caught exception: " << ex.what() << endl;  // prints a meaningful error message.
    }
    ...
    player.treat(
            City::MexicoCity);   // you now remove ALL disease cubes from MexicoCity, since there is a yelllow cure.
    ...
    cout << board.is_clean()
         << endl;  // prints "1" - the board is clean - congratulations!!! You treated all diseases!!!
}
